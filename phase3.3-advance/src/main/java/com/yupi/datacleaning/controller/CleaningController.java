...
public class CleaningController {
    ...
    public Map<String, Object> previewData(@PathVariable String taskId) {
        try {
            ...
            Map<String, Object> result = new HashMap<>();
            result.put("taskId", taskId);
            result.put("analysis", dataCleaner.analyzeData(table));
            result.put("sample", getSampleData(table));
            result.put("columns", table.columnNames());
    ...
    public void cleanupExpiredTasks() {
        log.info("Cleaning up expired tasks...");
        long now = System.currentTimeMillis();
        long expirationTime = 86400000L; // 24小时过期
        
        Iterator<Map.Entry<String, CleaningTask>> iterator = tasks.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<String, CleaningTask> entry = iterator.next();
            CleaningTask task = entry.getValue();
            if (now - task.getCreatedAt() > expirationTime) {
                log.info("Removing expired task: {}", task.getTaskId());
                iterator.remove();
                
                // 尝试删除临时文件
                try {
                    if (task.getOriginalFilePath() != null) {
                        Files.deleteIfExists(Paths.get(task.getOriginalFilePath()));
                    }
                    if (task.getCleanedFilePath() != null) {
                        Files.deleteIfExists(Paths.get(task.getCleanedFilePath()));
                    }
                } catch (Exception e) {
                    log.warn("Failed to delete file for task: {}", task.getTaskId(), e);
                }
            }
        }
    }
